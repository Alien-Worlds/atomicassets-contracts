...
namespace atomicdata {
    ...
    typedef std::variant<int8_t, int16_t, int32_t, int64_t, uint8_t, uint16_t, uint32_t, uint64_t, float, double,
        std::string, atomicdata::INT8_VEC, atomicdata::INT16_VEC, atomicdata::INT32_VEC, atomicdata::INT64_VEC,
    ...
    typedef std::map<std::string, atomicdata::ATOMIC_ATTRIBUTE> ATTRIBUTE_MAP;
    ...
    vector<uint8_t> serialize_attribute(const string &type, const ATOMIC_ATTRIBUTE &attr) {
        ...
        if (type == "int8") {
            ...
            check(DecodeBase58(std::get<string>(attr), result), "Error when decoding IPFS string");
